...
- Do not request administrator, superuser, master-key, kubeconfig, or cluster-admin credential values. Database DDL is executed manually by the Human Operator through an existing DBeaver connection; the project must not define, read, or request administrator environment variables or connection details.
...
- Keep PostgreSQL identities separated: `POSTGRES_LITELLM_*` is the least-privilege application identity for runtime DML. Administrator identity and credentials remain entirely outside the project in the Human Operator's DBeaver setup.
...
- Migrations and synchronization must be repeatable and non-destructive by default. `scripts/migrate.py` validates the checked-in SQL offline and must never connect to a database or claim to apply a migration. Manual migration SQL is transactional, records history only after success, contains no role/user/database values, placeholders, grants, or revokes, and must not alter unrelated LiteLLM objects. Runtime privileges are assigned by the Human Operator through DBeaver according to the documented capability allowlist.

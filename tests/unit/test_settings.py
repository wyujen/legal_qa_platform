...
def test_environment_contract_is_exactly_the_thirteen_runtime_names() -> None:
    expected = (
    ...
    assert RUNTIME_ENVIRONMENT_VARIABLES == expected
...
def test_settings_contract_does_not_declare_or_parse_admin_fields(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("UNDOCUMENTED_DATABASE_ADMIN_USER", "unit-test-operator-user")
    monkeypatch.setenv(
        "UNDOCUMENTED_DATABASE_ADMIN_PASSWORD",
        "unit-test-operator-password",
    )
    monkeypatch.setenv(
        "UNDOCUMENTED_DATABASE_ADMIN_DATABASE",
        "unit-test-operator-database",
    )
    ...
    assert not any("ADMIN" in name for name in DOCUMENTED_ENVIRONMENT_VARIABLES)
